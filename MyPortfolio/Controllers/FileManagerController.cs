...
namespace MyPortfolio.Controllers
{
    ...
    public class FileManagerController : ControllerBase
    {
        ...
        public FileManagerController(IExpenseRepo expenseRepo, IExpenseTypeRepo expenseTypeRepo,
            ...
            IAssetRepo assetRepo, IAssetCategoryRepo assetCategoryRepo, IAssetValueRepo assetValueRepo, IAssetOperationRepo assetOperationRepo)
        {
        ...
        [SwaggerOperation(Summary = "Add asset definition from file")]
        public async Task<IActionResult> AddAssetDefinitionFile(IFormFile file)
        {
            if (file == null || file.Length == 0)
            {
                return BadRequest("Nessun file fornito o file vuoto.");
            ...
            try
            {
                ...
                List<Asset> assetValueList = await AssetStaticUtils.ProcessAssetDefinitionFile(file, assetCategoryList);
                await _assetRepo.AddAssetListAsync(assetValueList);
                return Ok($"{assetValueList.Count} asset value added");
            }
            catch (Exception ex)
            {
